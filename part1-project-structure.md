# Task Manager (Python) — Discovery Document
...
Based only on file names, folder layout, and the README — before opening the source files properly.
...
1. Are `task_parser.py`, `task_priority.py`, and `task_list_merge.py` dead code, planned features, or used by something outside this repo?
2. Is there a plan to expose `task_priority.py`'s scoring in `list`/`stats`, since `list_tasks` doesn't currently sort by importance?
3. Is `task_list_merge.py` meant for a future multi-device/remote sync feature? What would "remote" be?
4. Why does `storage.py` rewrite the entire `tasks.json` file on every single write instead of an incremental approach — known scalability limit?
5. Why does `task_manager.py` import `argparse` when it's never used in that file?
